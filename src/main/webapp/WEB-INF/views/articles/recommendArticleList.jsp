	...
	<div class="container">
		...
		<div id="wrapper">
			...
			<div class="alert alert-error" align="center">
				...
				* 회원님께서 스크랩한 기사들과 다른 회원들이 스크랩한 기사들의 관계를 분석하여 추천 기사를 제공합니다.
			</div>
			
			<!-- start: 자료 유무 -->
			<c:if test="${empty requestScope.recommendArticleList}">
				<div align="center">
					<div class = "span6" style="float:none;" align="center">
						<hr>
						<h3 class="text-info"><i class="fa-icon-exclamation-sign"></i> 추천된 기사가 없습니다. </h3>
						<hr><hr>	
						<ul>
							<li>관심있는 기사들을 스크랩해보세요!</li>
							<li>다른 회원들이 스크랩한 기사들과 연관이 있으면 기사를 추천 해 줍니다.</li>
						</ul>	
						<hr>
					</div>
				</div>
			...
			<c:if test="${not empty requestScope.recommendArticleList}">
			<br>
				<div id="portfolio-wrapper" class="row-fluid isotope" style="position: relative; overflow: hidden; height: 1327px;">
					<c:forEach items="${requestScope.recommendArticleList}" var="scrap">
						<div class="span3 portfolio-item nature people isotope-item" style="position: absolute; left: 0px; top: 0px; -webkit-transform: translate3d(0px, 0px, 0px);">
							<div class="picture" style="border: 2px solid silver;" >
								<a class="articleView" value="${scrap.articleUrl}" title="기사보기">
								<c:if test="${not empty scrap.image}">
									<img src="${scrap.image}" alt="">
									<div class="image-overlay-link" style="opacity: 0; display: block;"></div>
								</c:if>	
								</a>
								<div class="item-description alt">
									<h5>
										<a class="articleView" value="${scrap.articleUrl}" style="color:#525252;font-weight:bolder;cursor: pointer;">[${scrap.articleSection}] ${scrap.title}</a>
									</h5>
								<p>${scrap.description}</p>
								<span>${scrap.writingDate}&nbsp;&nbsp;<font color="red">${scrap.press}</font></span>
								</div>
								<a class="scrap" style="color: brown; cursor: pointer; text-align:center;" value="${scrap.articleUrl}">[스크랩하기]</a>
							</div>
						</div>
					</c:forEach>
				</div>
			...
			$.ajax({
				...
				success : function(data) {
					...
					if(article != null){
						...
						$("#scrap").attr("value", article.articleUrl);
